...
'''
visualize_network: A function to visualize the grid network
@ Input:
G: A grid network

@ Output:
None. (Save the plot as: network_visualization.png)
'''

def visualize_network(G, N, pos, labels, V_f, filename = '00 Figures/network_visualization.png'):
    
    # Visualize the grid network
    edgecolors = ['orange' if G[u][v]['speed_limit'] < V_f else 'green' for u, v in G.edges()]
    plt.figure(figsize=(5,5))
    nx.draw(G, pos, labels=labels, node_size=200, node_color='lightgray', edge_color = edgecolors,
            with_labels=True, font_weight=500, connectionstyle ='arc3, rad=0.05')
...
def visualize_flow(G, paths, od_matrix, N, pos, labels, filename = '00 Figures/flow_visualization.png'):
...
def visualize_shortest_path(G, pos, shortest_paths, A, B, labels):
    ...
    plt.axis('off')
    fn = "00 Figures/shortestpath_" + str(A) + "_" + str(B) + ".png"
    plt.savefig(fn)
    #plt.show()